...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt, initial_response=None):
        """
        Evaluates a worker agent's response against criteria, with iterative refinement.

        Args:
            initial_prompt: The original prompt/query
            initial_response: Optional pre-generated response from worker agent.
                            If provided, skips calling respond() on first iteration.
        """
        client = OpenAI(api_key=self.openai_api_key)
        ...
        for i in range(self.max_interactions):
            ...
            # Step 1: Get worker's response (or use pre-generated one on first iteration)
            print(" Step 1: Worker agent generates a response to the prompt")
            if i == 0 and initial_response is not None:
                print(f"Using pre-generated response from knowledge agent")
                print(f"Prompt:\n{prompt_to_evaluate}")
